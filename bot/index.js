...
const isShyamAITrigger = () => false;
const handleShyamAI = async () => false;
const isShyamAIEnabled = () => false;
...
const {
  handleAutoReact
} = require("./shyam-cmds/automation/autoreactstatus.js");
const {
  handleChannelReact,
  discoverNewsletters,
  channelReactManager
} = require("./shyam-cmds/channel/channelreact.js");
const {
  handleReactOwner
} = require("./shyam-cmds/automation/reactowner.js");
const {
  handleReactDev
} = require("./shyam-cmds/automation/reactdev.js");
const {
  handleAutoView
} = require("./shyam-cmds/automation/autoviewstatus.js");
const {
  handleAutoDownloadStatus
} = require("./shyam-cmds/automation/autodownloadstatus.js");
const {
  initializeAutoJoin
} = require("./shyam-cmds/group/add.js");
const antidemote = require("./shyam-cmds/group/antidemote.js");
const {
  isBugMessage: antibugCheck,
  isEnabled: antibugEnabled,
  getAction: antibugGetAction
} = require("./shyam-cmds/group/antibug.js");
const {
...
} = require("./shyam-cmds/group/antilink.js");
const {
...
} = require("./shyam-cmds/group/antispam.js");
const banCommand = require("./shyam-cmds/group/ban.js");
const {
  setupWebServer,
  updateWebStatus
...
} = require("./shyam-cmds/group/antidemote.js");
const {
  handleGroupUpdate: antipromoteHandler
} = require("./shyam-cmds/group/antipromote.js");
const {
  isWelcomeEnabled,
  getWelcomeMessage,
  sendWelcomeMessage
} = require("./shyam-cmds/group/welcome.js");
const {
  isGoodbyeEnabled,
  getGoodbyeMessage,
  sendGoodbyeMessage
} = require("./shyam-cmds/group/goodbye.js");
const {
  handleStatusMention: statusMentionHandler
} = require("./shyam-cmds/group/antistatusmention.js");
const {
  setupAntiGroupStatusListener
} = require("./shyam-cmds/group/antigroupstatus.js");
const {
  setupAntiTagListener
} = require("./shyam-cmds/group/antitag.js");
const {
  setupAntiGroupMentionListener
} = require("./shyam-cmds/group/antigroupmention.js");
const {
  setupWordFilterListener
} = require("./shyam-cmds/group/wordfilter.js");
const {
  setupAutoReplyListener
} = require("./shyam-cmds/group/autoreply.js");
const {
  setupSlowModeListener
} = require("./shyam-cmds/group/slowmode.js");
const {
  isWhitelisted
} = require("./shyam-cmds/owner/whitelist.js");
const {
  isBlacklisted
} = require("./shyam-cmds/owner/blacklist.js");
...
} = require("./shyam-cmds/owner/antidelete.js");
...
} = require("./shyam-cmds/owner/antideletestatus.js");
const {
...
} = require("./shyam-cmds/ai/chatbot.js");
...
// const GROUP_NAME = 'SHYAM-MD Community';
...
const DiskManager = {
  ...
  getCleanupReport() {
    ...
    const tempFiles = this.getDirSize("./temp") + this.getDirSize("./shyam-cmds/temp");
  ...
  async cleanTempFilesAsync(aggressive = false) {
    let removed = 0;
    const tempDirs = ["./temp", "./temp/ig", "./temp/snapchat", "./temp/compressed", "./temp/apk", "./shyam-cmds/temp", "./viewonce_stealth", "./viewonce_downloads", "./temp_stickers", "./temp_url_uploads", "./collected_stickers", "./sticker_packs"];
    const now = Date.now();
...
//                 text: `🎉 *WELCOME TO SHYAM-MD!*\n\n` +
//                       `Thank you for connecting with SHYAM-MD! 🤖\n\n` +
//                       `✨ *Features Available:*\n` +
...
class AutoLinkSystem {
  ...
  async sendImmediateSuccessMessage(sock, senderJid, cleaned, isFirstUser = false) {
    try {
      const currentTime = new Date().toLocaleTimeString("en-US", { timeZone: process.env.TIME_ZONE || "Asia/kolkata" });
  ...
  async detectStatusUpdate(msg) {
    try {
      ...
      const logEntry = {
        ...
        postedAt: statusTime,
        detectedAt: new Date().toLocaleTimeString("en-US", { timeZone: process.env.TIME_ZONE || "Asia/kolkata" }),
        timestamp: Date.now()
      };
...
function setupHerokuSession() {
  try {
    ...
    if (herokuSessionId && herokuSessionId.trim() !== "") {
      ...
      // Parse DEX~ session format
      if (herokuSessionId.startsWith("DEX~:")) {
        UltraCleanLogger.info("🔐 Processing DEX~ session format...");
        try {
          // Remove SHYAM-MD: prefix and decode
...
class LoginManager {
  ...
  async selectMode() {
    console.log(chalk.yellow("\n🤖 SHYAM-MD v" + VERSION + " - LOGIN SYSTEM"));
  ...
  async sessionIdMode() {
    ...
    if (!sessionId || sessionId.trim() === "") {
      ...
      if (input.trim() === "1") {
        sessionId = await this.ask("Paste your Session ID (DEX~ or base64): ");
        if (!sessionId || sessionId.trim() === "") {
  ...
  async pairingCodeMode() {
    ...
    console.log(chalk.gray("Example: 917384287405"));
...
async function startBot(loginMode = "auto", loginData = null) {
  try {
    ...
    if (!initialCommandsLoaded) {
      ...
      commandLoadPromise = loadCommandsFromFolder("./shyam-cmds");
...
┃ 📢 Support Groups
┃ 🔗 Official Group 1
┃ https://chat.whatsapp.com/Fu8Ck4TetM8BX902ZEWVIR
┃
┃ 📣 WhatsApp Channels
┃ 🔗 Official Channel 1
...
async function handleConnectionCloseSilently(lastDisconnect, loginMode, phoneNumber) {
  ...
  if (loggedOut) {
    if (connectionAttempts >= 3) {
      ...
      UltraCleanLogger.info("💡 To fix: Get a new SESSION_ID → https://dex-pair-ai.onrender.com/session → set it in your .env → restart the bot.");
      return;
    }
    ...
    if (isshyamAIEnabled() && isShyamAITrigger(textMsg) && !msg.key.fromMe) {
      ...
      if (isOwnerW || isSudoW) {
        try {
          ...
          const executeShyamAICommand = async (cmdName, cmdArgs) => {
          ...
          const handled = await handleShyamAI(sock, msg, commands, executeShyamAICommand, textMsg);
          if (handled) {
            ...
            UltraCleanLogger.info(`🤖 Shyam AI: ${aiDisplay} ${aiLocTag} → "${aiPreview}"`);
            return;
          }
        } catch (aiErr) {
          UltraCleanLogger.error(`🤖 Shyam AI error: ${aiErr.message}`);
        }
      }
    }
    const currentPrefix = getCurrentPrefix();
        ...
        try {
          try {
            const _cmdEmojiMap = {
              ...
              chatbot: "💬", imagine: "🎨", makemusic: "🎵", shyam: "🤖",
              aibibl: "📖", aimuslim: "🕌",
              // Games
...
//         UltraCleanLogger.info(`🔐 Session ID support: ✅ ENABLED (SHYAM-MD: format)`);
